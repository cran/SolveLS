...
#'
#' @references
#' \insertRef{van_der_vorst_bi-cgstab:_1992}{SolveLS}
#'
...
lsolve.bicgstab <- function(A,B,xinit=NA,reltol=1e-5,maxiter=1000,
  ...
  if (any(is.infinite(A))||any(is.na(A))||any(is.infinite(B))||any(is.na(B))){
    stop("* lsolve.bicgstab : no NA, Inf, -Inf values are allowed.")
  }
  # 1-2. Size Argument
  m = nrow(A)
  if (is.vector(B)){
    mB = length(B)
    if (m!=mB){
      stop("* lsolve.bicgstab : a vector B should have a length of nrow(A).")
    }
  } else {
    mB = nrow(B)
    if (m!=mB){
      stop("* lsolve.bicgstab : an input matrix B should have the same number of rows from A.")
    }
  }
  if (is.vector(B)){
    B = as.matrix(B)
  ...
  if (m > ncol(A)){        ## Case 1. Overdetermined
    ...
    stop("* lsolve.bicgstab : underdetermined case is not supported.")
  }
  # 1-4. Preconditioner : only valid for square case
  if (!all.equal(dim(A),dim(preconditioner))){
    stop("* lsolve.bicgstab : Preconditioner is a size-matching.")
  }
  if (verbose){message("* lsolve.bicgstab : preprocessing finished ...")}
  ###########################################################################
  ...
  if (ncolB==1){
    ...
    for (i in 1:ncolB){
      ...
      if (verbose){
        message(paste("* lsolve.bicgstab : B's column.",i,"being processed.."))
      }
    }
  ...
  if ("flag" %in% names(res)){
    ...
    if (flagval==0){
      if (verbose){
        message("* lsolve.bicgstab : convergence well achieved.")
      }
    } else if (flagval==1){
      if (verbose){
        message("* lsolve.bicgstab : convergence not achieved within maxiter.")
      }
    } else {
      if (verbose){
  ...
  if (verbose){
    message("* lsolve.bicgstab : computations finished.")
  }
  return(res)
}
