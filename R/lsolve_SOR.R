...
#'
#' @references
#' \insertRef{demmel_applied_1997}{SolveLS}
#'
#' @rdname basic_SOR
#' @export
lsolve.sor <- function(A,B,xinit=NA,reltol=1e-5,maxiter=1000,w=1,adjsym=TRUE,verbose=TRUE){
  if (verbose){
  ...
  if (nrow(A)==ncol(A)){
    if (norm(abs(t(A)-A),"f")>1e-10){
      if (verbose){
        message("* lsolve.sor : A may not be symmetric.")
      }
      if (adjsym){
        B = t(A)%*%B
        A = t(A)%*%A
        if (verbose){
          message("* lsolve.sor : making it normal equation form via 'adjsym' flag.")
        }
      }
    }
  }
  # Preprocessing : SOR only : w
  if ((w<=0)||(w>=2)){
    stop("* lsolve.sor : weight value w should be in (0,2).")
  }
  if (w==1){
    if (verbose){
      message("* lsolve.sor : w=1 : it reduces to forward Gauss-Seidel method.")
    }
  }
  # Preprocessing : no NA or Inf
  if (any(is.infinite(A))||any(is.na(A))||any(is.infinite(B))||any(is.na(B))){
    stop("* lsolve.sor : no NA, Inf, -Inf values are allowed.")
  }
  ...
  if (is.vector(B)){
    mB = length(B)
    if (m!=mB){
      stop("* lsolve.sor : a vector B should have a length of nrow(A).")
    }
  } else {
    mB = nrow(B)
    if (m!=mB){
      stop("* lsolve.sor : an input matrix B should have the same number of rows from A.")
    }
  }
  if (is.vector(B)){
    B = as.matrix(B)
  ...
  n = ncol(A)
  if (m<n){
    stop("* lsolve.sor : underdetermined case is not supported.")
  } else if (m>n){
    B = (t(A)%*%B)
    A = (t(A)%*%A)
    if (verbose){
      message("* lsolve.sor : overdetermined case : turning into normal equation.")
    }
  }
  ...
  if (aux.is.dd(A)==FALSE){
    if (verbose){
      message("* lsolve.sor : LHS matrix A is not diagonally dominant.")
    }
  }
  ...
  if (ncolB==1){
    if (!sparseflag){
      vecB = as.vector(B)
      res = linsolve.sor.single(A,vecB,xinit,reltol,maxiter,w)
    } else {
      vecB = B
      res = linsolve.sor.single.sparse(A,vecB,xinit,reltol,maxiter,w)
    }
  } else {
    x      = array(0,c(n,ncolB))
    ...
    for (i in 1:ncolB){
      if (!sparseflag){
        vecB = as.vector(B[,i])
        tmpres = linsolve.sor.single(A,vecB,xinit,reltol,maxiter,w)
      } else {
        vecB = Matrix(B[,i],sparse=TRUE)
        tmpres = linsolve.sor.single.sparse(A,vecB,xinit,reltol,maxiter,w)
      }
      x[,i]       = tmpres$x
      iter[i]     = tmpres$iter
      errors[[i]] = tmpres$errors
      if (verbose){
        message(paste("* lsolve.sor : B's column.",i,"being processed.."))
      }
    }
  ...
  # Return
  if (verbose){
    message("* lsolve.sor : computations finished.")
  }
  return(res)
}
